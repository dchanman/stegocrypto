...
void data_transfer_receive(unsigned char *recv_msg) {
	...
	data_transfer_receive_communication(temp_msg, DATA_CONNECTION_CHECK_LENGTH, WAIT);

	//check for length of the string length of the final message
	printf("Waiting for length of string length message!\n");
	data_transfer_receive_communication(temp_msg, DATA_LENGTH_CONST, STRING_LENGTH);

	//check for length of the final message
	printf("Waiting for string length message!\n");
	data_transfer_receive_communication(temp_msg, recv_msg_length, MESSAGE_LENGTH);

	//check for file
	printf("Waiting for file!\n");
	recv_msg = malloc(sizeof(char) * actual_msg_length);
	data_transfer_receive_communication(recv_msg, actual_msg_length, FINAL);
}
 ...
 *
 * @param imagedata - pointer to a message that will be sent
 * @param datalength - length of message
 */
void data_transfer_send(unsigned char *filename) {
	...
	data_transfer_send_communication(length_of_send_msg_length, sizeof(length_of_send_msg_length), STRING_LENGTH);
	printf("Length of string length message sent!\n");

	//send length of the final message
	data_transfer_send_communication(send_msg_length, sizeof(send_msg_length), MESSAGE_LENGTH);
	printf("String length message sent!\n");

	//send final message
	data_transfer_send_communication(imagedata, imagelength, FINAL);
...
void data_transfer_send(unsigned char *imagedata, const int datalength) {
	...
	//send length of the string length of the final message
	data_transfer_send_communication(length_of_send_msg_length, strlen(length_of_send_msg_length), STRING_LENGTH);
	printf("Length of string length message sent!\n");

	//send length of the final message
	data_transfer_send_communication(send_msg_length, strlen(send_msg_length), MESSAGE_LENGTH);
	printf("String length message sent!\n");

	//send final message
	data_transfer_send_communication(imagedata, datalength, FINAL);
	printf("Final message sent!\n");
...
static void data_transfer_send_communication(unsigned char *msg, const int length, const int com_stage) {
	...
	memset(ready_check, '\0', DATA_CONNECTION_CHECK_LENGTH);
